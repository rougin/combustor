 ...
 * @Entity
 * @Table(
 *      name="{{ name }}"{{ indexes | length > 0 ? ',' : ''}}
{% if indexes | length == 1 %}
 *      indexes={"{{ indexes[0] }}"}
{% elseif indexes | length > 1 %}
 *      indexes={
{% for index in indexes %}
 *          @Index(name="{{ index }}", columns={"{{ index }}"}){{ loop.index < loop.length ? ',' : '' }}
{% endfor %}
 *      }
{% endif %}
 * )
