...
<?= form_open('posts/edit/' . $item->get_id()) ?>
  <?= form_hidden('_method', 'PUT') ?>

  <div class="mb-3">
    <?= form_label('Title', '', ['class' => 'form-label mb-0']) ?>
    <?= form_input('title', set_value('title', $item->get_title()), 'class="form-control"') ?>
    <?= form_error('title', '<div><span class="text-danger small">', '</span></div>') ?>
  </div>

  <div class="mb-3">
    <?= form_label('Text', '', ['class' => 'form-label mb-0']) ?>
    <?= form_input('text', set_value('text', $item->get_text()), 'class="form-control"') ?>
    <?= form_error('text', '<div><span class="text-danger small">', '</span></div>') ?>
  </div>

  <div class="mb-3">
    <?= form_label('User', '', ['class' => 'form-label mb-0']) ?>
    <?= form_dropdown('user_id', $users, set_value('user_id', $item->get_user_id()), 'class="form-control"') ?>
